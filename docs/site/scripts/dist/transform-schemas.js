...
async function loadFromCache() {
  ...
  const versions = await (0, import_promises.readdir)(CACHE_DIR);
  if (versions.length === 0) {
    return void 0;
  }
  return Promise.all(
    versions.map(async (version) => {
      const configurationSchema = await (0, import_promises.readFile)(
        (0, import_path.join)(CACHE_DIR, version, "configurationSchema.json"),
        "utf8"
      );
      return {
        version,
...
// scripts/transform-schemas.ts
var import_dotenv = __toESM(require_main());
(0, import_dotenv.config)();
getReleases().then(
  (releases) => releases.map(async (release) => {
  })
);
/*!
