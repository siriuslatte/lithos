...
generous free tier which you are very unlikely to reach the limits of while using Lithos, but you
...
`{your-name}-lithos-states` and select an AWS region that is near your location. Ensure "Block _all_
public access" is checked to keep your data private.

In the future, Lithos may support object locking, so you may want to expand "Advanced settings" and
...
Rename your local state file to `{project-name}.lithos-state.yml`, then upload it to your bucket.
