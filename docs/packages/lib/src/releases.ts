...
const GITHUB_OWNER = 'siriuslatte';
const GITHUB_REPO = 'lithos';
...
async function loadFromGitHub(): Promise<Release[]> {
  ...
  const githubReleases: any[] = await client.paginate(
    client.rest.repos.listReleases as any,
    repoParams
  );
  ...
  return (
    await Promise.all(
      githubReleases.map(async (githubRelease: any) => {
        const asset_id = githubRelease.assets.find(
          (asset: any) => asset.name === 'schema.json'
        )?.id;
...
export async function getReleases() {
  ...
  if (releases.length === 0) {
    const fallback = await loadBundledSchema();
    if (fallback) {
...
async function loadBundledSchema(): Promise<Release | undefined> {
  ...
  const schemaPath = join(workspaceDir, '..', 'specs', 'schema.json');
  if (!existsSync(schemaPath)) {
    return undefined;
  }

  console.log(chalk`{grey Loading bundled schema from} ${schemaPath}`);

  const configurationSchema = JSON.parse(await readFile(schemaPath, 'utf8'));
  const transformSchema = getTranslateToNextraTransformer();
  await transformSchema(configurationSchema);
...
async function getNextRelease(): Promise<Release> {
  ...
  const cargoWorkspaceDir = join(workspaceDir, '..');

  console.log(chalk`{grey Generating} next {grey from} ${cargoWorkspaceDir}`);

  const rawSchema = await new Promise<string>((resolve, reject) => {
    exec('cargo run --bin gen_schema', { cwd: cargoWorkspaceDir }, (err, stdout) => {
      if (err) {
        reject(err);
      }
      resolve(stdout);
